;;; unrepl-overlay.el --- General Overlays Functionality -*- lexical-binding: t; -*-
;; 
;; Filename: unrepl-overlay.el
...
(defun unrepl--remove-result-overlay ()
  "Remove result overlay from current buffer.
This function also removes itself from `post-command-hook'."
  (remove-hook 'post-command-hook #'unrepl--remove-result-overlay 'local)
  (remove-overlays nil nil 'category 'result))


(defun unrepl--remove-result-overlay-after-command ()
  "Locally add `unrepl--remove-result-overlay' to `post-command-hook'.
This function also removes itself from `post-command-hook'."
  (remove-hook 'post-command-hook #'unrepl--remove-result-overlay-after-command 'local)
  (add-hook 'post-command-hook #'unrepl--remove-result-overlay nil 'local))


(defun unrepl--make-overlay (start end type &rest props)
...
PROPS is a plist of properties and values to add to the overlay."
  (let ((o (make-overlay start end (current-buffer))))
    ...
    (push #'unrepl--delete-overlay (overlay-get o 'modification-hooks))))


(defun unrepl--make-result-overlay (value &optional where duration type
                                          &rest props)
...
This function takes some optional arguments:
  ...
  `unrepl-eval-overlay-result-duration' variable.
  TYPE is passed to `unrepl--make-overlay' (defaults to `result').
      ...
      (save-excursion
        ...
        (let* ((type (or type 'result))
               ...
               (display-string (format " %s%s "
                                       unrepl-eval-result-prefix
                                       value))
               (o nil))
          (remove-overlays beg end 'category type)
          (funcall (if unrepl-overlays-use-font-lock
                       #'font-lock-prepend-text-property
                     #'put-text-property)
                   0 (length display-string)
                   'face 'unrepl-result-overlay-face
                   display-string)
          ...
          (setq o (apply #'unrepl--make-overlay
                         ...
                         props))
          (pcase (or duration unrepl-overlay-eval-result-duration)
            ((pred numberp) (run-at-time duration nil #'unrepl--delete-overlay o))
            ('command
             ;; If inside a command-loop, tell `unrepl--remove-result-overlay'
             ;; to only remove after the *next* command.
             (if this-command
                 (add-hook 'post-command-hook
                           #'unrepl--remove-result-overlay-after-command
                           nil 'local)
               (unrepl--remove-result-overlay-after-command))))
...
(provide 'unrepl-overlay)

;;; unrepl-overlay.el ends here
